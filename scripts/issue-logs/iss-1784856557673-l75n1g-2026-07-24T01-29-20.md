...
The agent confirms exactly what I diagnosed: the button at `/app/web/src/App.tsx:501` has `className="btn btn--ghost btn--sm offcanvas-session-row__delete"`. The `.btn` base class (line 1009) applies `border: 1px solid var(--border-strong)`, and the `.offcanvas-session-row__delete` override was missing `border: none;`. The fix is already in place — no further changes needed.

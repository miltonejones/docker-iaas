# Issue iss-1784862029148-erxesk

**Summary:** missing icon
**Category:** bug
**Reported:** 2026-07-24T03:00:29.148Z
**Exit code:** 0
...
### Details
  ...
  description: the Create an issue link in the footer needs an icon
```
...
**Root cause:** The "Create an issue" button in the app footer (`web/src/App.tsx:622`) was plain text with no icon, unlike every other comparable element in the UI:

- The GitHub link in the **same footer** has `<AppIcon name="github" />`
- The mobile bottom-nav "Ask" button has `<AppIcon name="assistant" />`
- The CreateIssueModal heading has `<AppIcon name="warning" />`

The `bug` icon (`faBug`) was already available in the icon map (`web/src/icons.tsx`) and already used for the Issues sidebar navigation item, making it the natural choice.

**Fix:** Added `<AppIcon name="bug" />` before the "Create an issue" text on line 622, matching the `<AppIcon name="github" /> GitHub` pattern already used by the adjacent footer link.

```json
{"changedFiles":["web/src/App.tsx"],"rootCause":"The 'Create an issue' footer button had no icon while the adjacent GitHub link and all other 'create issue' UI elements elsewhere in the app had icons.","diagnosis":"Added <AppIcon name=\"bug\" /> before the 'Create an issue' text in the footer button at App.tsx:622, matching the pattern already used by the GitHub link in the same footer.","confidence":"high"}
```

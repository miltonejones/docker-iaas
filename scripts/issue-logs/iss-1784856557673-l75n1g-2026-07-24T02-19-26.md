# Issue iss-1784856557673-l75n1g

**Summary:** .offcanvas-session-row__delete button has unwanted border from .btn rule
**Category:** bug
**Reported:** 2026-07-24T01:29:17.673Z
**Exit code:** 0
...
### Details
  ...
  description: The `.offcanvas-session-row__delete` button should have no border, but a `.btn` selector with `border-width: 1px` is applying a border to it. The fix should override or remove the border on `.offcanvas-session-row__delete` specifically. Related to iss-1784853801839-92v18u which identified the symptom but not the root cause.
  relatedIssue: iss-1784853801839-92v18u
```
...
**Diagnosis:** The delete button inside the offcanvas sessions panel is rendered with `className="btn btn--ghost btn--sm offcanvas-session-row__delete"` (`web/src/App.tsx:501`). The `.btn` base class applies `border: 1px solid var(--border-strong)`, which gives the button a visible border. The `.btn--ghost` modifier only changes the border color but doesn't remove the border. The `.offcanvas-session-row__delete` rule previously only had `flex-shrink: 0` — nothing to suppress the border from `.btn`.

**Fix:** Added `border: none;` to the `.offcanvas-session-row__delete` rule in `web/src/styles.css:2901`. This explicitly overrides the border inherited from `.btn`, making the delete button borderless as intended.

```json
{"changedFiles":["web/src/styles.css"],"rootCause":"The delete button has class `btn` which applies `border: 1px solid var(--border-strong)`. The `.btn--ghost` variant only changes border-color but doesn't remove the border. The `.offcanvas-session-row__delete` rule lacked a `border: none` override.","diagnosis":"Button element at App.tsx:501 has `btn btn--ghost btn--sm offcanvas-session-row__delete`. The `.btn` base rule (line 1009) applies a 1px solid border. `.btn--ghost` changes border-color but preserves the border. `.offcanvas-session-row__delete` only set `flex-shrink: 0` without removing the unwanted border.","confidence":"high"}
```

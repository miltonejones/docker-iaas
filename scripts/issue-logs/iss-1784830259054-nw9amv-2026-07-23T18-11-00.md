# Issue iss-1784830259054-nw9amv

**Summary:** Footer nav
**Category:** general
**Reported:** 2026-07-23T18:10:59.054Z
**Exit code:** 0
...
### Details
  ...
  description: Remove the databases button from the footer nav
```
...
**Issue:** The "Databases" button needed to be removed from the footer navigation (the mobile bottom tab bar, `.bottom-nav`).

**Root cause:** The bottom nav iterated over the full `SERVICES` array, which includes `{ path: '/databases', label: 'Databases', icon: 'database' }`. This array is also used by the `ServiceNav` dropdown and `Breadcrumbs` component, so removing the entry from the array outright would break those.

**Fix:** Added a `.filter((s) => s.path !== '/databases')` before the `.map()` in the bottom nav rendering at `web/src/App.tsx:637`. This removes only the Databases button from the mobile footer tab bar while leaving the `SERVICES` array untouched for all other consumers.

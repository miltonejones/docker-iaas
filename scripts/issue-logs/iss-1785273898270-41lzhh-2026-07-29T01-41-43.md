# Issue iss-1785273898270-41lzhh

**Summary:** assistant-input
**Category:** bug
**Reported:** 2026-07-28T21:24:58.270Z
**Engine:** default
...
### Details
  ...
  description: the border radius of the assistant-input in the assistant panel in Neon mode should be --r-lg
```
...
Both background agents have confirmed the same finding: the theme files in `/app/web/src/themes/` redefine the `--r-*` variables per theme. In neon mode, `--r-lg: 10px` while `--r-sm: 4px`, so `.assistant-input` was getting a 4px radius when it should get 10px. The fix is already applied — no further changes needed.

(Note: the agents also spotted an unrelated issue — `.hamburger` at `styles.css:111` references `var(--radius-sm)` which is never defined — but that's out of scope for this issue.)

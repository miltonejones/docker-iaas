...
The background agent confirms: the button at `App.tsx:501` has `className="btn btn--ghost btn--sm offcanvas-session-row__delete"` — exactly as diagnosed. The fix is already applied and the agent's own read of the CSS shows the updated rules with `border: none;` in place.

**Summary of changes:**

In `/app/web/src/styles.css`, the `.offcanvas-session-row__delete` rule was extended from:
...
.offcanvas-session-row__delete:hover:not(:disabled) {
  color: var(--danger);
  background: var(--crit-soft);
}
```

This removes the leaked border from `.btn` and adds a red-tinted hover state consistent with every other delete button in the UI (e.g., `.assistant-sessions-panel__delete`).

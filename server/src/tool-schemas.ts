...
const GATEWAY_TOOLS: ToolSchema[] = [
  ...
  },
  {
    ...
    properties: {
      ...
      domain: { type: 'string', description: 'Custom domain to assign at creation (optional)' },
    },
    required: ['name', 'targetType', 'targetId'],
  },
  {
    name: 'update_gateway_route',
    description: 'Update a gateway route. Pass any fields you want to change; omit to keep unchanged.',
    properties: {
      id: { type: 'string', description: 'Route ID' },
      displayName: { type: 'string', description: 'New display name' },
      targetPort: { type: 'number', description: 'New target port (container routes only)' },
      method: { type: 'string', description: 'New HTTP method filter' },
      pathPattern: { type: 'string', description: 'New URL path pattern' },
      domain: { type: 'string', description: 'New custom domain (or null to clear)' },
    },
    required: ['id'],
  },
  {
...
const GITHUB_TOOLS: ToolSchema[] = [
  ...
  },
  {
    ...
    properties: {
      ...
      path: { type: 'string', description: 'Destination path in container' },
      ref: { type: 'string', description: 'Git ref (branch, tag, or commit SHA)' },
      clean: { type: 'boolean', description: 'Delete destination first (direct strategy only)' },
      strategy: { type: 'string', enum: ['direct', 'atomic'], description: 'Direct or atomic (staging + mv swap, no 502 gap)' },
    },

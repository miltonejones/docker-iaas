...
export async function run(presetName: string, containerId: string, destinationPath: string) {
  const destination = containerPath(destinationPath);
  if (typeof presetName !== 'string' || typeof containerId !== 'string' || !destination) {
    throw new HttpError(400, 'preset, id, and a valid absolute destination path are required.');
  }

  const preset = listHostBuildPresets().find((item) => item.name === presetName);
  if (!preset) {
    throw new HttpError(404, `Build preset "${presetName}" is not configured.`);
  }
  const container = docker.getContainer(containerId);
  const info = await container.inspect();
  if (info.Config?.Labels?.['iaas.system']) {
    throw new HttpError(403, 'This container is system-managed and cannot receive build artifacts.');
  }
  if (!info.State?.Running) {
    throw new HttpError(409, 'Container is not running — start it before deploying build artifacts.');
  }
  ...
  return { ok: true, preset: preset.name, id: containerId, path: destination, size: artifacts.size };
}

...
export async function copyToBucket(
  ...
  if (typeof bucket !== 'string' || !bucket || typeof key !== 'string' || !key) {
    throw new HttpError(400, 'bucket and key are required.');
  }
  ...
  return { bucket, key, size: file.content.length };
}
...
): Promise<{ ok: boolean; id: string; path: string; size: number }> {
  const relativePath = validContainerPath(destinationPath);
  if (typeof containerId !== 'string' || !containerId || !relativePath) {
    throw new HttpError(400, 'id and a valid absolute destination path are required.');
  }

  const container = docker.getContainer(containerId);
  const info = await container.inspect();
  if (info.Config?.Labels?.['iaas.system']) {
    throw new HttpError(403, 'This container is system-managed and cannot be written to here.');
  }
  if (!info.State?.Running) {
    throw new HttpError(409, 'Container is not running — start it before copying files.');
  }

  const file = await readHostFile(sourcePath);
  await container.putArchive(await archiveFile(relativePath, file.content), { path: '/' });
  recordAuditLog('host_file.copy_to_container', 'host', null, null, `${containerId}:${destinationPath}`);
  return { ok: true, id: containerId, path: destinationPath, size: file.content.length };
}
